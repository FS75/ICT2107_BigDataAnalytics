...
public class WordCountReviewDescriptionsMapper extends Mapper<LongWritable, Text, Text, LongWritable> {
	...
	protected void setup(Context context) throws IOException, InterruptedException {
	 ...
	 @Override
	    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			// get the text fields
			String reviewFieldText = fields[2];

			// split the text fields into words and count each word
			countWords(reviewFieldText, context);
		}
		...
		private void countWords(String text, Context context) throws IOException, InterruptedException {
		    ...
		    for (String word : words) {
		        Text outputKey = new Text(company + "|" + platform + "|ReviewDescriptions|"+ word);
		...
		protected void cleanup(Context context) throws IOException, InterruptedException {
			...
			for (Map.Entry<String, Long> entry : wordCounts.entrySet()) {
				Text outputKey = new Text(company + "|" + platform + "|ReviewDescriptions|" + entry.getKey());
