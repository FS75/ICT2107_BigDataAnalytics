...
public class WordCountAnalysis {
	public static void main(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "WordCountReviewDescriptions");

		job.setJarByClass(WordCountAnalysis.class);

		// One reducer to give a combined result
		job.setReducerClass(WordCountReducer.class);
		job.setMapperClass(WordCountReviewDescriptionsMapper.class);
		...
		Path outputPath = new Path(
				"hdfs://hadoop-master:9000/user/ict2101702/reviewDescriptions/" + new Date().getTime());
